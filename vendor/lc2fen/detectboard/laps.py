...
# Get the directory where this laps.py script is located
_LAPS_PY_DIR = Path(__file__).resolve().parent
# Construct the full path to the model
_LAPS_MODEL_PATH = _LAPS_PY_DIR / "models" / "laps_model.onnx"

# Ensure the models directory and the model file exist before loading
if not _LAPS_MODEL_PATH.exists():
    # This is a fallback in case the model is missing.
    # In a real application, you might need to download it or ensure it's packaged correctly.
    raise FileNotFoundError(
        f"LAPS model not found at the expected path: {_LAPS_MODEL_PATH}. "
        "Please ensure the file 'laps_model.onnx' is in the 'vendor/lc2fen/detectboard/models/' directory."
    )

__LAPS_SESS = onnxruntime.InferenceSession(str(_LAPS_MODEL_PATH))
